...
Citizen.CreateThread(function()
    for k, _ in pairs (Config.Stores) do
        ...
        if Config.Stores[k].shopType == "barber" then
            ...
            BeginTextCommandSetBlipName("STRING")
            AddTextComponentString("Фризьорски Салон")
        ...
        clothingCombo:onPlayerInOut(function(isPointInside, _, zone)
            if isPointInside then
                ...
                if zoneName == 'surgeon' then
                    --exports['qb-core']:DrawText('[E] - Пластична Операция', 'left')
                    exports['qb-target']:OpenDraw('Пластична Операция', 'E')
                elseif zoneName == 'clothing' then
                    --exports['qb-core']:DrawText('[E] - Магазин за дрехи', 'left')
                    exports['qb-target']:OpenDraw('Магазин за дрехи', 'E')
                elseif zoneName == 'barber' then
                    --exports['qb-core']:DrawText('[E] - Фризьорски Салон', 'left')
                    exports['qb-target']:OpenDraw('Фризьорски Салон', 'E')
                end
            else
                inZone = false
                --exports['qb-core']:HideText()
                exports['qb-target']:CloseDraw()
            end
        end)
...
function enableCam()
    ...
    if(not DoesCamExist(cam)) then
        ...
        --SetCamCoord(cam, coords.x, coords.y, coords.z + 0.2) // old
...
-- Fixing Character Hair and Skin  Bug
...
local function setPedHeadBlend(ped, headBlend)
    ...
    if isPedFreemodeModel(ped) then
        SetPedHeadBlendData(ped,
            ...
            headBlend.shapeThird or 0,
            headBlend.skinFirst or 0,
            headBlend.skinSecond or 0,
            headBlend.skinThird or 0,
...
function resetClothing(data)
    ...
    -- Face
    setPedHeadBlend(ped, {
        shapeFirst = skinData["face"].item,
        shapeSecond = skinData["face2"].item,
        skinFirst = skinData["face"].texture,
        skinSecond = skinData["face2"].texture,
        shapeMix = skinData["facemix"].shapeMix,
        skinMix = skinData["facemix"].skinMix
    })
