...
QBClothing.ResetItemTexture = function (obj, category) {
    ...
    $.post('https://ls-clothing/updateSkin', JSON.stringify({
...
$(document).on('click', '.clothing-menu-option-item-right', function (e) {
    ...
    if (canChange) {
        if (hasTracker && clothingCategory == "accessory") {
            $.post('https://ls-clothing/TrackerError');
            return
        } else {
            if (clothingCategory == "model") {
                $(inputElem).val(newValue);
                $.post('https://ls-clothing/setCurrentPed', JSON.stringify({ ped: newValue }), function (model) {
...
$(document).on('change', '.item-number', function () {
    ...
    if (hasTracker && clothingCategory == "accessory") {
        ...
        if (clothingCategory == "accessory" && inputVal == 13) {
            ...
            $.post('https://ls-clothing/updateSkinOnInput', JSON.stringify({
                clothingType: clothingCategory,
...
$(document).on('click', '.clothing-menu-header-camera-btn', function (e) {
    ...
    if (selectedCam == null) {
        ...
        $.post('https://ls-clothing/setupCam', JSON.stringify({
            value: camValue
        }));
        selectedCam = this;
    } else {
...
$(document).on('keydown', function() {
    switch(event.keyCode) {
        case 68: // D
            $.post('https://ls-clothing/rotateRight');
            break;
        case 65: // A
            $.post('https://ls-clothing/rotateLeft');
            break;
        case 87: // W
            $.post('https://ls-clothing/CamUp');
            break;
        case 83: // S
            $.post('https://ls-clothing/CamDown');
            break;            
    }
});
...
$(document).on('click', "#save-menu", function (e) {
    e.preventDefault();
    QBClothing.Close();
    $.post('https://ls-clothing/saveClothing');
});

$(document).on('click', "#cancel-menu", function (e) {
    e.preventDefault();
    QBClothing.Close();
    $.post('https://ls-clothing/resetOutfit');
});
...
QBClothing.Open = function (data) {
    ...
    $(".clothing-menu-container").css({ "display": "block" }).animate({ right: '12.9vh', }, 200);
    ...
    $.each(data.menus, function (i, menu) {
        if (menu.selected) {
            ...
            // Toggle backgroundkur visibility
            $('.backgroundkur').css("display", "block");
...
$(document).on('click', '.clothing-menu-outfit-option-button', function (e) {
    ...
    $.post('https://ls-clothing/selectOutfit', JSON.stringify({
...
$(document).on('click', '.clothing-menu-myOutfit-option-button-remove', function (e) {
    ...
    $.post('https://ls-clothing/removeOutfit', JSON.stringify({
        outfitData: outfitData.skin,
        outfitName: outfitData.outfitname,
        outfitId: outfitData.outfitId,
    }));
...
QBClothing.Close = function () {
    ...
    selected = null;
    lastCategory = null;
    $(".clothing-menu-container").css({ "display": "block" }).animate({ right: "-500px", }, 200, function () {
        $(".clothing-menu-container").css({ "display": "none" });
    });

    // Hide backgroundkur when closing
    $('.backgroundkur').css("display", "none");
}
...
$(document).on('click', '#save-outfit-save', function (e) {
    ...
    $.post('https://ls-clothing/saveOutfit', JSON.stringify({
...
$(document).on('click', '.change-camera-button', function (e) {
    ...
    $.post('https://ls-clothing/rotateCam', JSON.stringify({
        type: rotationType
    }))
});
        ...
        $(document).mousemove(function(e) {
            ...
            if (e.pageX < mX) {
                    if($("#rotate-char:hover").length != 0){
                        $.post('https://ls-clothing/rotateLeftChar');
                    }
                 // moving character right
            } else {
                    if($("#rotate-char:hover").length != 0){
                        $.post('https://ls-clothing/rotateRightChar');
                    }
            }
            mX = e.pageX
        });
...
$(document).ready(function() {
    $('#rotate-char').bind('mousewheel', function(e){
        if(e.originalEvent.wheelDelta /120 > 0) {
            ...
            $.post('https://ls-clothing/fovup');
        }
        else{
            // console.log('scrolling down !');
            $.post('https://ls-clothing/fovdown');
        }
    });
});